...
*** Setup
       ...
       - for consistency, the jupyter and R kernels should be
         ...
         - so prepare an executable shell script at
           ...
           #+END_SRC
*** Usage
       1. ssh into the VM, e.g. with the above convenient setup
          #+begin_src shell
            ssh vm
          #+end_src
       2. go to the project directory
       3. start jupyter with dependencies using =jrwork=
       4. once jupyter is started, note the port number of the url,
          e.g. if the link is
          "http://localhost:8890/?token=d4bfa08127d33e9e09820ef3fc3c135ac6086f78e459fe2e",
          the port *inside VM* is 8890
       5. do ssh port forwarding
          ...
          - the general syntax of ssh port forwarding into the VM is
            =ssh -L xxx:localhost:yyy=, where =xxx= is the local port
            number you like, and =yyy= is the port number inside the
            VM, so for the above example of port 8890 and map to the
            same port, we would use
            #+begin_src shell
              ssh -L 8890:localhost:8890
            #+end_src
       6. browse the url for jupyter
...
*** Usage
       ...
       2. open terminal, ssh into vm by =ssh vm=, e.g. by using ctrl+alt+t configured above
       3. go to desired project directory, run =rwork= (for plain R) or =rdwork= (for radian)
       4. open files to edit, can send text to terminal for evaluation
       5. when done, just exit R and ssh in the terminal
** Emacs
*** Idea
