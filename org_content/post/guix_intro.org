...
commands, you should get the /same/ outputs.
...
#+begin_src shell
  ...
  # clone the repo
  cd ~
  git clone git@github.com:peterloleungyau/guix_demo.git
#+end_src

Or you may choose to use =guix environment= to tempoarily install
=git= just for the cloning:
#+begin_src shell
  cd ~
  guix environment --ad-hoc git -- git clone git@github.com:peterloleungyau/guix_demo.git
#+end_src
